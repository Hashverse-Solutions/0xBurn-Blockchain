...
contract NFTMarketplace is Ownable,ReentrancyGuard,IERC721Receiver {
    ...
    */
    ...
    function createMarketSaleby0xBurn(
        ...
        if(!listNfts.ownerListNFT){
            ...
            uint256 totalPercet = DAOTreasuryAmount.sub(teamAmount).sub(RewardPoolAmount).sub(DAOTreasuryAmount).sub(PartnerNFTAmount);
    ...
    */
    ...
    function placeBidby0xBurn(address _tokenContract, uint256 _tokenId, address owner, uint256 _amount) external nonReentrant{
        Auction storage auction = auctions[_tokenContract][_tokenId][owner];
        require(auction.duration > 0, "Auction does not exist");
        require(block.timestamp < auction.duration, "Auction has ended");
        require(_amount > auction.startPrice, "Bid amount must be higher");
        require(_amount > highestBids[_tokenContract][_tokenId][owner].amount, "Bid amount must be higher");

        IERC20(Address0xBurn).transferFrom(msg.sender,address(this),_amount);
        ...
        // Refund the previous bidder
        if (previousBid.bidder != address(0)) {
            IERC20(Address0xBurn).transfer(previousBid.bidder,previousBid.amount);
        }
    }
    ...
    */
    ...
    function endAuctionby0xBurn(address _tokenContract, uint256 _tokenId, address owner) external nonReentrant {
        ...
        if(!auction.ownerListNFT){
            IERC20(Address0xBurn).transfer(auction.seller, totalPrice);
        }else{
            ...
            IERC20(Address0xBurn).transfer(DAOTreasury,DAOTreasuryAmount);
            IERC20(Address0xBurn).transfer(teamAddress,teamAmount);
            IERC20(Address0xBurn).transfer(RewardPool,RewardPoolAmount);

            transferHoldersby0xBurn(PartnerNFTAmount);

            IERC20(Address0xBurn).transfer(address(0),founderAmount);
        }
